...
namespace Texture
{
				 ...
				 float length)
  {
    ...
    assert(density > 0);
    const int number = 1 + int(_width * density);
    const int ilength = 1 + int(_width * length);
  ...
  void		Channel::CellsGen(unsigned int cells, unsigned int n)
  {
	...
	const float pixel = sqrtf((float)distance);
	(*this)[i + shift] = pixel;
	if (pixel > max)
	  max = pixel;
      }
...
// A deplacer autre part ?
  ...
  {
    int xi0 = int(x0 * _width);
    int yi0 = int(y0 * _height);
    int xi1 = int(x1 * _width);
    int yi1 = int(y1 * _height);
  ...
  void		Channel::Dilate(float radius)
  {
    ...
    const float invMaxDistance = 1.f / sqrtf(float(iradius * iradius));
	...
	for (int kj = 0; kj < iradius; ++kj)
	  ...
	  {
	    const float factor = 1.f - fmin(1.f, sqrtf(float(ki * ki + kj * kj)) * invMaxDistance);
	    maxValue = fmax(maxValue, factor * save[GetIndex(i + ki, j + kj)]);
	    maxValue = fmax(maxValue, factor * save[GetIndex(i - ki, j + kj)]);
	    maxValue = fmax(maxValue, factor * save[GetIndex(i + ki, j - kj)]);
	    maxValue = fmax(maxValue, factor * save[GetIndex(i - ki, j - kj)]);
	  }
  ...
  void		Channel::Translate(float hshift, float vshift)
  {
    ...
    const int ihshift = (unsigned int)(_width * hshift);
    const int ivshift = (unsigned int)(_height * vshift);
